...
RSpec.describe Conversation do
  ...
  describe 'open conversation uniqueness' do
    let(:account) { create(:account) }
    let(:inbox) { create(:inbox, account: account) }
    let(:contact) { create(:contact, account: account) }
    ...
    it 'rejects a second open conversation for the same contact and inbox on create' do
      ...
      expect(duplicate.errors[:base].first).to start_with(described_class::OPEN_DUPLICATE_MESSAGE_PREFIX)
    end

    it 'allows a new open conversation when the previous one is resolved' do
    ...
    it 'allows a new open conversation when the previous one is snoozed' do
      create(
        :conversation,
        account: account,
        inbox: inbox,
    ...
    it 'reuses the existing open conversation when skip flag is set and one already exists' do
      existing = create(:conversation, account: account, inbox: inbox, contact: contact, contact_inbox: contact_inbox, status: :open)

      result = described_class.create_with_open_duplicate_recovery!(base_attrs)
      expect(result.id).to eq(existing.id)
      expect(described_class.where(contact_id: contact.id, inbox_id: inbox.id, status: :open).count).to eq(1)
    end

    it 'ends with a single open row when multiple workers insert concurrently' do
      allow(Rails.configuration.dispatcher).to receive(:dispatch)

      threads = 8.times.map do
        Thread.new do
          ActiveRecord::Base.connection_pool.with_connection do
            described_class.create_with_open_duplicate_recovery!(base_attrs)
          end
        end
      end
      threads.each(&:join)

      opens = described_class.where(contact_id: contact.id, inbox_id: inbox.id, status: :open)
      expect(opens.count).to eq(1)
    end
  end
end
