  ...
  describe 'POST /api/v1/accounts/{account.id}/conversations' do
    ...
    context 'when it is an authenticated user' do
      ...
      context 'when it is an authenticated user who has access to the inbox' do
        ...
        it 'returns 422 when the contact already has an open conversation in that inbox' do
          allow(Rails.configuration.dispatcher).to receive(:dispatch)
          assignee = create(:user, account: account, role: :agent, name: 'Carlos Souza')
          create(
            :conversation,
            account: account,
            inbox: inbox,
          ...
          post "/api/v1/accounts/#{account.id}/conversations",
               headers: agent.create_new_auth_token,
               params: { source_id: contact_inbox.source_id, contact_id: contact.id, inbox_id: inbox.id },
               as: :json

          expect(response).to have_http_status(:unprocessable_entity)
          body = JSON.parse(response.body, symbolize_names: true)
          expect(body[:error]).to include('Carlos Souza')
        end

        it 'creates a new conversation when the only existing one for contact/inbox is snoozed' do
          allow(Rails.configuration.dispatcher).to receive(:dispatch)
          create(
            :conversation,
            account: account,
