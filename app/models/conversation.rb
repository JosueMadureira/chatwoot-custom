...
class Conversation < ApplicationRecord
  ...
  OPEN_DUPLICATE_MESSAGE_PREFIX = 'Este contato já possui um atendimento em aberto'.freeze

  attr_accessor :skip_open_duplicate_validation
  ...
  validate :no_duplicate_open_conversation, on: :create, unless: :skip_open_duplicate_validation
  ...
  def self.open_duplicate_error_message(existing_conversation)
    assignee_label = duplicate_open_assignee_label(existing_conversation)
    "Este contato já possui um atendimento em aberto com o agente #{assignee_label}"
  end

  def self.duplicate_open_assignee_label(existing_conversation)
    if existing_conversation.assignee.present?
      existing_conversation.assignee.available_name
    elsif existing_conversation.assignee_agent_bot.present?
      existing_conversation.assignee_agent_bot.name.presence || 'bot'
    else
      'sem agente designado'
    end
  end

  # Retorna true se, após as mesmas regras de determine_conversation_status, o status seria open.
  def self.preview_will_be_open_after_creation_rules?(record)
    preview = record.dup
    preview.send(:apply_creation_status_rules)
    preview.open?
  end

  # Usado em canais de entrada que criam conversa sob concorrência: garante recuperação se o índice único falhar.
  def self.create_with_open_duplicate_recovery!(attributes)
    record = new(attributes)
    record.skip_open_duplicate_validation = true
    record.save!
    record
  rescue ActiveRecord::RecordNotUnique => e
    raise e unless open_contact_inbox_unique_violation?(e)

    find_by!(
      contact_id: attributes[:contact_id] || attributes['contact_id'],
      inbox_id: attributes[:inbox_id] || attributes['inbox_id'],
      status: :open
    )
  end

  def self.open_contact_inbox_unique_violation?(exception)
    cause = exception.cause
    return false unless defined?(PG::UniqueViolation) && cause.is_a?(PG::UniqueViolation)

    cause.message.include?('index_conversations_open_unique_per_contact_and_inbox')
  end
  ...
  def determine_conversation_status
    apply_creation_status_rules
  end

  def apply_creation_status_rules
    if contact.blocked?
  ...
  def no_duplicate_open_conversation
    return unless self.class.preview_will_be_open_after_creation_rules?(self)

    existing = self.class.includes(:assignee, :assignee_agent_bot).find_by(
    ...
    errors.add(:base, self.class.open_duplicate_error_message(existing))
  end
