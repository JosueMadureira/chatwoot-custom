...
class Notification < ApplicationRecord
  ...
  def message_preview(actor)
    return 'Nova mensagem recebida' if actor.blank?

    content = message_content(actor)

    if content.blank?
      return actor.try(:attachments)&.present? ? 'Enviou um anexo' : 'Nova mensagem recebida'
    end

    transform_user_mention_content(content.truncate(100, omission: '…'))
  end
